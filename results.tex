...
provides a clear, consistent velocity advantage (9.3\% reduction for Original; 2.1\% for CBAM).
For SR, this advantage effectively vanishes for the CBAM architecture: configurations~G (masked)
and~H (synthetic noise) differ in velocity MSE by only 0.2\%, with~H (0.04833) marginally
outperforming~G (0.04844) on total, $v$, and $w$ components.  This implies that the
pre-upsampling attention mechanism enables the network to achieve competitive intraluminal
velocity accuracy from a raw, unmasked input --- without requiring any vascular segmentation at
inference time --- when the task complexity is high enough (SR) to activate the full benefit of
the attention module.  The trade-off is worse geometric fidelity (HD: 13.59 vs.\ 9.50\,mm) and
higher background noise ($1.42$ vs.\ $0.67{\times}10^{-3}$), both of which are consequences of
the absence of the zeroed-background input prior.  For applications where a CoW mask is readily
available, masked-input CBAM (G) offers the best balance of velocity accuracy and background
suppression.  For fully mask-free pipelines, CBAM with synthetic-noise augmentation (H) is the
recommended configuration, as it achieves velocity accuracy within 0.2\% of its masked
counterpart while operating on unprocessed 3T inputs.
