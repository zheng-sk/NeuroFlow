...
Ten configurations were evaluated on a held-out test subject (case~000) not seen during
training.  Eight span the full factorial combination of two task modes, two network
architectures, and two input conditioning strategies; the ninth and tenth are two-stage cascade
pipelines described below.  In \textbf{Denoising} mode ($\times 1$),
...
Configuration~\textbf{I} is an \textbf{independent two-stage cascade}: the SR model~H
(CBAM-PreUpsampling, synthetic-noise, $\times 2$) is applied to the downsampled 3T input;
the resulting SR prediction is re-segmented to produce an automatic CoW mask; the masked SR
output is then passed to the Denoising model~A (Original, masked, $\times 1$), which was trained
on registered 3T data.  Configuration~\textbf{J} is a \textbf{fine-tuned cascade}: the same
frozen SR stage~1 (model~H) is used, but the Denoising stage~2 was subsequently fine-tuned on
SR-stage outputs rather than original 3T acquisitions, explicitly resolving the domain mismatch
present in configuration~I.  Both cascades test whether chaining a mask-free SR model with a
masked Denoising model can recover the performance of pure masked training without requiring a
pre-existing segmentation of the original 3T scan.
...
for intraluminal velocity, $\times 10^{-3}$ for extravascular background).  Configurations~I
and~J are grouped together as two-stage cascade pipelines in all tables.

\begin{table}[!htbp]
  \centering
  \caption{Summary of the ten evaluated configurations.}
  ...
  \begin{tabular}{clll}
    ...
    \midrule
    I & \makecell[l]{SR ($\times 2$) $\to$ Denoising ($\times 1$)\\(independent cascade)} &
      \makecell[l]{CBAM-PreUpsampling (SR)\\+ Original (Denoising)} &
      \makecell[l]{Synth.\ noise (SR) $\to$\\auto-seg.\ $\to$ Masked (Dn.)} \\
    J & \makecell[l]{SR ($\times 2$) $\to$ Denoising ($\times 1$)\\(fine-tuned cascade)} &
      \makecell[l]{CBAM-PreUpsampling (SR, frozen)\\+ Original (Denoising, fine-tuned)} &
      \makecell[l]{Synth.\ noise (SR) $\to$\\auto-seg.\ $\to$ Masked (fine-tuned Dn.)} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{cll l rrrr}
    ...
    J & \makecell[l]{CBAM-PreUps.\ (SR, frozen)\\+ Original (Dn., fine-tuned)} & \makecell[l]{Synth.\ noise $\to$\\auto-seg.\ $\to$ Masked} & & $4.973$ & $1.575$ & $1.920$ & $1.478$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Configuration~I (independent cascade) yields the highest total velocity MSE of any configuration
...
most affected ($2.072{\times}10^{-2}$, the worst among all ten configurations), consistent with
the cascade compounding the left--right noise floor already present in the SR stage.

Configuration~J (fine-tuned cascade) demonstrates that fine-tuning the Denoising stage on SR
outputs directly resolves this domain mismatch.  Its total velocity MSE of $4.973{\times}10^{-2}$
represents a 3.5\% improvement over the independent cascade~I ($5.154{\times}10^{-2}$), and
closely matches the single-stage SR models (E: $4.977{\times}10^{-2}$, F: $4.983{\times}10^{-2}$).
The $v$-direction shows the most dramatic recovery: $1.920{\times}10^{-2}$ for~J vs.\
$2.072{\times}10^{-2}$ for~I, a 7.3\% reduction.  That said, the fine-tuned cascade does not
surpass the best single-stage Denoising models (A: $4.281{\times}10^{-2}$), suggesting that the
two-stage SR-then-Denoising pipeline introduces irreducible velocity error from the first
stage that fine-tuning alone cannot eliminate.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{cll l r rrr}
    ...
    J & \makecell[l]{CBAM-PreUps.\ (SR, frozen)\\+ Original (Dn., fine-tuned)} & \makecell[l]{Synth.\ noise $\to$\\auto-seg.\ $\to$ Masked} & & 18{,}005 & 0.684 & 0.760 & 13.55 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Configuration~I achieves the worst Dice of all ten configurations (0.680), despite its
...
Configuration~J (fine-tuned cascade) shows that fine-tuning the Denoising stage does not
improve geometric fidelity: its Dice (0.684), sMSD (0.760\,mm), and HD (13.55\,mm) are
virtually identical to those of configuration~I.  This confirms that the geometric bottleneck
in the cascade lies not in the domain mismatch of the Denoising stage, but in the accuracy of
the intermediate CoW mask derived from the SR output.  Since the same frozen SR model and the
same auto-segmentation step are used in both I and J, both pipelines inject the same geometric
errors at the stage~2 input boundary.  Fine-tuning the Denoising weights improves its ability
to handle SR-domain inputs, but cannot correct the spatial layout of a mask it receives
as-is.  Improving cascade geometry would therefore require either a more accurate intermediate
segmentation or an end-to-end trainable boundary that avoids an explicit binary masking step
altogether.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{cll l rrrr}
    ...
    J & \makecell[l]{CBAM-PreUps.\ (SR, frozen)\\+ Original (Dn., fine-tuned)} & \makecell[l]{Synth.\ noise $\to$\\auto-seg.\ $\to$ Masked} & & $\mathbf{0.57{\times}10^{-3}}$ & $\mathbf{0.19{\times}10^{-3}}$ & $\mathbf{0.25{\times}10^{-3}}$ & $\mathbf{0.13{\times}10^{-3}}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Configuration~J (fine-tuned cascade) achieves the lowest background MSE of both cascades
and indeed of all mask-free-input models: $0.57{\times}10^{-3}$, within the range of the
single-stage masked models (A: $0.64$, C: $0.61$, E: $0.78$, G: $0.67{\times}10^{-3}$).
This striking result --- better background suppression than any unmasked single-stage model ---
reflects two compounding benefits: the fine-tuned Denoising stage has learned, on SR-domain
inputs, that regions outside the auto-generated mask should produce near-zero predictions, while
simultaneously having been exposed during fine-tuning to the typical mis-segmentations produced
by the SR-derived mask, making it more robust to mask boundary leakage.  The $v$-component
improvement is particularly pronounced ($0.25$ vs.\ $0.42{\times}10^{-3}$ in~I), consistent
with the left--right phase channel being most sensitive to domain adaptation.  These findings
demonstrate that fine-tuning directly addresses the domain mismatch for the background channel
even though it cannot correct the geometric inaccuracies in the intermediate mask.

% -------------------------------------------------------
\subsection*{4.5 Consolidated Comparison}
% -------------------------------------------------------

Table~\ref{tab:summary} consolidates the five primary performance indicators across all ten
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{cll l rrrrr}
    ...
    J & \makecell[l]{CBAM-PreUps.\ (SR, frozen)\\+ Original (Dn., fine-tuned)} & \makecell[l]{Synth.\ noise $\to$\\auto-seg.\ $\to$ Masked} & & $4.973$ & 0.684 & 0.760 & $13.55$ & $\mathbf{0.57}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Fine-tuning resolves domain mismatch in the cascade but cannot overcome the geometric
bottleneck imposed by the intermediate binary mask.}
Configuration~I was designed to combine the mask-free flexibility of the SR model~H with the
refinement quality of the masked Denoising model~A, but instead achieves the worst velocity MSE
of all ten configurations ($5.154{\times}10^{-2}$), the worst Dice (0.680), and degraded
background suppression ($0.84{\times}10^{-3}$).  Two compounding failure modes are identified:
(i)~\emph{domain mismatch} --- model~A was trained on registered 3T acquisitions and receives
SR-enhanced predictions as input, placing it outside its training distribution; and
(ii)~\emph{mask error propagation} --- the intermediate CoW segmentation derived from the SR
output embeds the geometric errors of stage~1 directly into the masked input of stage~2.
Configuration~J demonstrates that \emph{fine-tuning} directly and selectively corrects
failure mode~(i): velocity MSE improves from $5.154$ to $4.973{\times}10^{-2}$ ($-3.5\%$) and
background MSE drops from $0.84$ to $0.57{\times}10^{-3}$, the best background suppression of
any mask-free pipeline.  However, fine-tuning leaves failure mode~(ii) entirely unchanged:
J's geometry (Dice: 0.684, HD: 13.55\,mm) is nearly identical to~I (Dice: 0.680, HD:
13.54\,mm), confirming that the mask accuracy is the exclusive geometric bottleneck.  The
practical implication is that a cascade architecture requires both (a) domain-adapted stages
\emph{and} (b) a higher-quality or continuous-valued intermediate segmentation to achieve
competitive geometry.  Replacing the binary auto-segmentation step with a soft, differentiable
mask probability map --- either from a learned segmentation network or by propagating the SR
model's internal attention activations --- is the most direct avenue for addressing the
remaining cascade geometry deficit.
