...
def load_case_table(csv_path: str, include_hr_mag: bool = True) -> List[Dict[str, Any]]:
    from neuroflow.data.NiftiPatchDataset import load_nifti_case_table
...
def load_sr_model(
    ...
    use_seg_head: Optional[bool] = None,
):
    ...
    # Accept both the current key and the pre-rename "cascade_use_seg_head",
    # so checkpoints trained before the rename still resolve the seg head.
    if use_seg_head is None and isinstance(checkpoint, dict):
        for key in ("use_seg_head", "cascade_use_seg_head"):
            if key in checkpoint:
                use_seg_head = bool(checkpoint[key])
                break
    if use_seg_head is None:
        use_seg_head = False
    ...
    model = build_sr_model(
        ...
        use_seg_head=bool(use_seg_head),
    ).to(device)
    ...
    setattr(model, "use_seg_head", bool(use_seg_head))
