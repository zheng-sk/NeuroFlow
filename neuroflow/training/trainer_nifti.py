...
from neuroflow.data.NiftiPatchDataset import create_nifti_full_volume_dataloader, create_nifti_patch_dataloader
from neuroflow.training.TrainerController import TrainerController
from neuroflow.models.model_factory import available_model_variants, normalize_model_variant
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Aliases: baseline, identity_skip, local_attention, attention_before_upsample."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Attach the auxiliary segmentation head to the CBAM model "
            "(pre_upsample_attention). --cascade-use-seg-head is a deprecated alias."
        ),
    )
    parser.add_argument(
        "--seg-loss-weight",
    ...
    parser.add_argument(
        ...
        dest="seg_head_bias_init",
        type=float,
        default=-4.0,
        help=(
            "Initial bias for the final segmentation-head conv (negative favours a "
            "sparse foreground). --cascade-seg-head-bias-init is a deprecated alias."
        ),
    )
    ...
    network = TrainerController(
        ...
        seg_head_bias_init=args.seg_head_bias_init,
