...
- loads Vx, Vy, Vz and Magnitude NIfTI files,
- computes speed = sqrt(Vx^2 + Vy^2 + Vz^2),
- computes PC-MRA = Magnitude * speed,
- writes a standardized output folder for downstream steps.
"""
...
def save_nifti(data: np.ndarray, affine: np.ndarray, header, out_path: str) -> None:
    image = nib.Nifti1Image(data.astype(np.float32, copy=False), affine, header)
    nib.save(image, out_path)


def process_case(vx_path: str, vy_path: str, vz_path: str, mag_path: str, output_dir: str, case_id: str) -> bool:
    ...
    try:
        vx_img = nib.load(vx_path)
        vy_img = nib.load(vy_path)
        vz_img = nib.load(vz_path)
    ...
    shutil.copy2(vx_path, os.path.join(output_dir, "Vx.nii.gz"))
    shutil.copy2(vy_path, os.path.join(output_dir, "Vy.nii.gz"))
    shutil.copy2(vz_path, os.path.join(output_dir, "Vz.nii.gz"))

    affine = vx_img.affine
    header = vx_img.header.copy()
    save_nifti(mag, affine, header, os.path.join(output_dir, "input_mag_raw.nii.gz"))
    save_nifti(speed, affine, header, os.path.join(output_dir, "input_speed_raw.nii.gz"))
    save_nifti(pcmra, affine, header, os.path.join(output_dir, "input_pcmra_raw.nii.gz"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Output subdirectory inside --data-root (default: processed_inputs)",
    )
...
def main() -> None:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        out_dir = os.path.join(args.data_root, args.output_subdir, case_id)
        ok = process_case(
